'''
For pair-formed datasets, e.g., snli, mnli, qnli, rte.
'''
...
from utils import (
    print_args,
    shuffle_dataset_binary, shuffle_dataset_triple,
    id2word_agnews, word2id_agnews,
    id2word_ethos_liar, word2id_ethos_liar,
    word2id_qnli_rte, id2word_qnli_rte,
    word2id_snli_mnli, id2word_snli_mnli,
    get_num_classes
)


from template_snli_mnli import solve_prompt_template, feedback_prompt_agnews, instruction0_agnews
...
class ResPromptBooster():
    def __init__(self, args, train_dataset, valid_dataset, test_dataset):
        ...
        self.max_wrong_example = args.max_wrong_example
        ...
        self.k = min(args.fewshot_k, self.num_train)
        self.train_X = list(zip(train_dataset[0][:self.k], train_dataset[1][:self.k]))
        self.train_y = train_dataset[2][:self.k]
        self.test_X = list(zip(test_dataset[0][:self.num_test], test_dataset[1][:self.num_test]))
        self.test_y = test_dataset[2][:self.num_test]
        ...
        if self.dataset_name == 'agnews':
            ...
            self.id2word = id2word_agnews
        elif self.dataset_name in ('ethos', 'liar'):
            self.word2id = word2id_ethos_liar
            self.id2word = id2word_ethos_liar
        elif self.dataset_name in ('qnli', 'rte'):
            self.word2id = word2id_qnli_rte
            self.id2word = id2word_qnli_rte
        elif self.dataset_name in ('snli', 'mnli'):
            self.word2id = word2id_snli_mnli
            self.id2word = id2word_snli_mnli
        else:
    ...
    def solve_instance_single(self, solve_prompt, ins):
        ...
        '''
        assert '{text1}' in solve_prompt and '{text2}' in solve_prompt
        conversion = {"appId": APPID}
        try:
            messages = [
                {
                    "role": "user",
                    "content": solve_prompt.replace('{text1}', ins[0]).replace("{text2}", ins[1])
                }
            ]
    ...
    def feedback_generate(self, wrong_indices, labels, y_pred, old_solve_prompt):
        ...
        wrong_X = [self.train_X[idx]    for idx in list(wrong_indices)][:self.max_wrong_example]
        wrong_C = [id2text(y_pred[idx]) for idx in list(wrong_indices)][:self.max_wrong_example]
        right_C = [id2text(labels[idx]) for idx in list(wrong_indices)][:self.max_wrong_example]
    ...
    def final_evaluate(self):
        ...
        test_pred = []
        for p in self.used_solve_prompts:
            _, _, labels, y_pred, _, acc, precision, recall, f1 = self.solve_task_single(p, self.test_X, self.test_y, mode='test')
...
if __name__ == '__main__':
    ...
    elif args.dataset in ('ethos', 'liar', 'rte', 'snli', 'mnli', 'qnli'):
